defmodule BetterMe.Seeds.RoutineTemplates do
  @moduledoc "Seed global routine templates (user_id: nil)."

  alias BetterMe.Repo
  alias BetterMe.Workouts.Schema.{RoutineDay, RoutineExercise, RoutineTemplate}
  import Ecto.Query

  def run do
    IO.puts("\n--- Seeding routine templates ---")
    seed_min_max_5x()
  end

  defp seed_min_max_5x do
    case Repo.one(from t in RoutineTemplate, where: t.name == "Min-Max 5x" and is_nil(t.user_id)) do
      nil ->
        ...
        Enum.each(days(), &insert_day(template, &1))
  ...
  defp insert_day(template, day_data) do
    {:ok, day} =
